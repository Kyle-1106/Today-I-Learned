...
# 2024年10月
## Springを用いたアプリケーション設計
### Springプロジェクトの叩き台作成のために必要な知識の習得
APIをSpringで実装するPJの叩き台作成を任されたため、そのために必要な知識について学んだ。
具体的には
・Controller,Servie,Repositoryではそれぞれどのようなことを実装するべきか
・それぞれの親クラスでは何を実装するべきか
・共通クラスはどのような観点で分けるべきか
・定数はどのような形で持つべきか
などである。今まではアプリケーション設計ということであくまで概念的なものを学んでいたがこのタイミングで実際のソースコードに落とし込むことで、それぞれのクラスの役割がより理解できた。

### SpringSecurityのキャッチアップ
今までSpringを触った経験はあるが、SpringSecurityの認証ロジックを実装したことはなかったので学んだ。
